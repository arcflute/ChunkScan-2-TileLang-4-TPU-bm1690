"""P5 BM1690 TileLang-TPU ChunkScan configuration candidates.
...
The factory preserves the complete S0 ABI and math while exposing two bounded
configuration dimensions used by P5: the paper's sProg-A/sProg-B producer
order and two/three software-pipeline stages.  It intentionally keeps the
K16-by-four reduction shape frozen; insufficient-loop-depth K32/K64 and
four-stage configurations are rejected by the P5 configuration gate before
kernel construction.
"""
...
def make_chunk_scan_pipeline_p5_kernel(schedule: str, num_stages: int):
    """Return one legal P5 K16 pipeline candidate."""
    ...
    def main_kernel_inner(
        ...
        with T.Kernel(1, 1, is_cpu=True) as (_, __):
            ...
            for chunk in T.serial(NCHUNKS):
                ...
                # Prepare only the strict-upper-triangle correction.  Each
                # P5 candidate obtains the useful dense cb tile from a
                # versioned GDMA load inside the pipeline; dense - upper is
                # the exact lower-triangular causal tile.  This avoids
                ...
                for k_blk in T.Pipelined(
                    ...
                    if schedule == SPROG_A:
                        # Paper Fig. 6 sProg-A gives LoadX producer priority.
                        T.ppl_copy(
                            x[
                                chunk * CHUNK_SIZE
                                + k_blk * REDUCE_TILE,
                    ...
                    # sProg-B gives the transformed LoadY path priority and
                    # delays LoadX until immediately before the scan GEMM.
                ...
                # P5 retains the same residual placement as S1/S2/S3; only
                # the causal-scan configuration differs.
