"""Validate the P3 reduction-tiled serial ChunkScan S1 with BM1690 cmodel."""
...
ARTIFACT_DIR = ROOT / "artifacts" / "reduction_serial"
S0_RUNTIME_DIR = ARTIFACT_DIR / "runtime_s0_control"
S1_RUNTIME_DIR = ARTIFACT_DIR / "runtime_s1"
S0_RESULT_PATH = (
    REPOSITORY_ROOT
    / "tpu_demo"
    / "mamba2_chunk_scan"
...
EXPECTED_REDUCTION_LOOP = "for (int k_blk = 0; k_blk < 4; ++k_blk)"
...
def _archive_lowering(program, toolchain: Dict[str, object]) -> Dict[str, object]:
    ...
    manifest = {
        "variant": "reduction_tiled_serial_s1",
        "serial_control": "serial_s0",
        "logical_abi": [
            "cb",
            "x",
            "dt",
...
def _archive_s1_cmodel_sources() -> None:
    for source_name, archive_name in (
        ...
        source = S1_RUNTIME_DIR / source_name
        if not source.is_file():
            raise FileNotFoundError(source)
...
def _case_result(
    actual_s0: torch.Tensor,
    actual_s1: torch.Tensor,
    inputs: Inputs,
    name: str,
) -> Dict[str, object]:
    expected = chunk_scan_reference(*inputs)
    s0_stats = comparison_stats(actual_s0, expected)
    s1_stats = comparison_stats(actual_s1, expected)
    cross_stats = comparison_stats(actual_s1, actual_s0)
    s0_close = bool(
        torch.allclose(actual_s0.float(), expected.float(), atol=ATOL, rtol=RTOL)
    )
    s1_close = bool(
        torch.allclose(actual_s1.float(), expected.float(), atol=ATOL, rtol=RTOL)
    )
    cross_close = bool(
        torch.allclose(actual_s1.float(), actual_s0.float(), atol=ATOL, rtol=RTOL)
    )
    ...
    result = {
        "case": name,
        "s0_close_to_oracle": s0_close,
        "s1_close_to_oracle": s1_close,
        "s1_close_to_s0": cross_close,
        "s0_vs_oracle": s0_stats,
        "s1_vs_oracle": s1_stats,
        "s1_vs_s0": cross_stats,
    ...
    if (
        ...
        or not s1_close
        or not cross_close
        or s0_stats["nan_count"]
        or s1_stats["nan_count"]
        or result["expected_nonzero_count"] == 0
        or any(value == 0.0 for value in chunk_max_abs)
    ):
        raise AssertionError(f"P3 S1 mismatch for {name}: {result}")
    return result


def test_reduction_serial_chunk_scan_cmodel() -> None:
    toolchain = assert_chunkscan_toolchain()
    print(json.dumps(toolchain, indent=2, sort_keys=True), flush=True)

    if not S0_RESULT_PATH.is_file():
        raise FileNotFoundError(
            f"missing closed P2 serial evidence: {S0_RESULT_PATH}"
        )
    s0_result = json.loads(S0_RESULT_PATH.read_text(encoding="utf-8"))
    if s0_result.get("status") != "PASS":
        raise AssertionError(f"P2 serial control is not PASS: {s0_result}")

    s1_program = make_chunk_scan_reduction_serial_kernel()
    lowering = _archive_lowering(s1_program, toolchain)
    ...
    if actual_counts != EXPECTED_SOURCE_SITE_COUNTS:
        raise AssertionError(f"unexpected P3 S1 source structure: {actual_counts}")
    if raw_source.count(EXPECTED_REDUCTION_LOOP) != 1:
        raise AssertionError("P3 S1 does not contain one serial four-iteration K loop")
    if REDUCE_TILES < 4:
        raise AssertionError("P3 requires at least four reduction iterations")
    if metrics["static_local_memory_end_bytes"] > metrics["bm1690_local_memory_limit_bytes"]:
        raise AssertionError(f"P3 S1 exceeds LMEM: {metrics}")

    s0_outputs = _run_worker("s0")
    ...
    results["residual_only_negative_D"] = _case_result(
        s0_outputs["residual_only_negative_D"],
    ...
    causal_base = _variant_inputs(base, "scan_only")
    clean_s0 = s0_outputs["causal_clean"]
    poisoned_s0 = s0_outputs["causal_poisoned"]
    clean_s1 = s1_outputs["causal_clean"]
    poisoned_s1 = s1_outputs["causal_poisoned"]
    expected = chunk_scan_reference(*causal_base)
    causal_result = {
        "bitwise_equal_clean_vs_poisoned_s0": bool(
            torch.equal(clean_s0, poisoned_s0)
        ),
        "bitwise_equal_clean_vs_poisoned_s1": bool(
            torch.equal(clean_s1, poisoned_s1)
        ),
        "poisoned_s1_close_to_oracle": bool(
            torch.allclose(
                poisoned_s1.float(), expected.float(), atol=ATOL, rtol=RTOL
            )
        ),
        "poisoned_s1_close_to_s0": bool(
            torch.allclose(
                poisoned_s1.float(), poisoned_s0.float(), atol=ATOL, rtol=RTOL
            )
        ),
        "poisoned_s1_vs_oracle": comparison_stats(poisoned_s1, expected),
        "poisoned_s1_vs_s0": comparison_stats(poisoned_s1, poisoned_s0),
    }
    if not all(
        (
            causal_result["bitwise_equal_clean_vs_poisoned_s0"],
            causal_result["bitwise_equal_clean_vs_poisoned_s1"],
            causal_result["poisoned_s1_close_to_oracle"],
            causal_result["poisoned_s1_close_to_s0"],
        )
    ):
        raise AssertionError(f"P3 S1 causal poisoning failed: {causal_result}")
    ...
    if cmodel_source != archived_raw_source:
        raise AssertionError("serial S1 raw and cmodel sources unexpectedly differ")

    summary = {
        "status": "PASS",
        "variant": "reduction_tiled_serial_s1",
        "serial_control_evidence": {
            "path": str(S0_RESULT_PATH),
            "status": s0_result["status"],
            "comparison_method": (
                "S0 and S1 run in separate cmodel worker processes, then the "
...
if __name__ == "__main__":
    if len(sys.argv) == 3 and sys.argv[1] == "--worker":
        ...
        test_reduction_serial_chunk_scan_cmodel()
    else:
        raise SystemExit(f"Usage: {sys.argv[0]} [--worker s0|s1]")
